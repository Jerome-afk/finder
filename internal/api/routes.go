...
func SetupRoutes(
        ...
        router.GET("/discover", webHandler.DiscoverPage)
        router.GET("/movie/:id", webHandler.MovieDetailPage)
        router.GET("/tv/:id", webHandler.TVDetailPage)

        // Auth routes (both /auth and /api/auth for compatibility)
        ...
        apiAuth := router.Group("/api/auth")
        {
                ...
                apiAuth.GET("/user", middleware.AuthRequired(), authHandler.GetCurrentUser)
        }
